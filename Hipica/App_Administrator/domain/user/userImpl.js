...
define(function userImplModule() {
    ...
    function userImpl(currentUser) {
        ...
        if (currentUser) {
            id(ko.isObservable(currentUser.id) ? currentUser.id()
                : currentUser.id);
            version(ko.isObservable(currentUser.version) ? currentUser.version()
                : currentUser.version);
            ...
            oldPassword(ko.isObservable(currentUser.oldPassword) ? currentUser.oldPassword()
                : currentUser.oldPassword);
            newPassword(ko.isObservable(currentUser.newPassword) ? currentUser.newPassword()
                : currentUser.newPassword);
            confirmNewPassword(ko.isObservable(currentUser.confirmNewPassword) ? currentUser

...
(function($) {
    $.fn.outerHtml =
        function() {
            ...
            return "<" + name + (attrs.length > 0 ? " " + attrs.join(" ") : "") + ">"
                + elem.innerHTML + "</" + name + ">";
        };
})(jQuery);
...
ko.bindingHandlers.progress = {
    init : function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        $element.attr('id', guid()).addClass('progress progress-info').append(bar);
...
ko.bindingHandlers.alert =
    {
        init : function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            $element.addClass('alert alert-' + alertInfo.priority).append(dismissBtn).append(
                alertMessage);
        }
    };
...
ko.bindingHandlers.popover = {
    init : function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        // Need to copy this, otherwise all the popups end up with the value of
        // the last item
        ...
        $(element).bind(trigger, function() {
            ...
            if ($('#' + domId).is(':visible')) {
                ...
                /*
                 * Since bootstrap calculates popover position before template
                 * is filled, a smaller popover height is used and it appears
                 * moved down relative to the trigger element. So we have to fix
                 * the position after the bind
                 */
                ...
                case 'right':
                    popover.offset({
                        top : triggerElementPosition - popoverHeight / 2 + triggerElementHeight / 2
                    });
                    break;
                case 'top':
                    popover.offset({
                        top : triggerElementPosition - popoverHeight - arrowSize
                    });
                    break;
                case 'bottom':
                    popover.offset({
                        top : triggerElementPosition + triggerElementHeight + arrowSize
                    });
                }
            }
        ...
        // Also tell KO *not* to bind the descendants itself, otherwise they
        // will be bound twice
        return {

...
define(
    ...
    function athleteHorses(config, i18n, findRequestImpl, pageImpl, pagerImpl, pageRequestImpl,
                          horseBroker, horseFilterImpl, horseSortImpl, horseImpl, app, shell, athleteImpl,
                          athleteViewModel) {
        "use strict";

        // state definition
        var viewModel = $.extend(false, {}, athleteViewModel), superActivate =
            viewModel.activate, PAGE_SIZE = config.PAGE_SIZE, PAGE_SIZES = config.PAGE_SIZES, availableProvinces =
            ko.observable(), nextFilter = ko.observable(horseFilterImpl()), currentFilter =
            horseFilterImpl(), currentSort = ko.observable(horseSortImpl()), currentPage =
            ko.observable(pageImpl()), currentPager = ko.observable(pagerImpl()), currentPageSize =
            ko.observable(PAGE_SIZE);

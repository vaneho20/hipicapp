define(['durandal/composition', 'durandal/system', 'durandal/viewModel'],
        ...
        frontPage.addContext('default', {
            ...
            addHost: function (modal) {
                ...
                if (frontpage.length === 0) {
                    frontpage = $('.root');
                    nextClass = 'root covered';
                    origClass = '';
                }

                var blockout = $('<div class="modalBlockout"></div>')
                    .css({ 'z-index': frontPage.getNextZIndex(), 'opacity': this.blockoutOpacity })
                    .appendTo(stacked);

                frontpage.removeClass(origClass).addClass(nextClass);

                var host = $('<div class="front animated enterFromRight"></div>')
                    .css({ 'z-index': frontPage.getNextZIndex() })
                    .appendTo(stacked);
                ...
                if (!frontPage.isModalOpen()) {
                    modal.oldBodyMarginRight = $("body").css("margin-right");

                    var html = $("html");
                    var oldBodyOuterWidth = frontpage.outerWidth(true);
                    var oldScrollTop = html.scrollTop();
                    $("html").css("overflow-y", "hidden");
                    var newBodyOuterWidth = $("body").outerWidth(true);
                    frontpage.css("margin-right", (newBodyOuterWidth - oldBodyOuterWidth + parseInt(modal.oldBodyMarginRight)) + "px");

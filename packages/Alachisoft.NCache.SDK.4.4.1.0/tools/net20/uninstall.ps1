...
if($osType -eq "64-bit")
{
	$buildEventVal=$buildEventVal.Replace("`r`nxcopy `"$installPath\additionalLib\Oracle20\x64\Oracle.DataAccess.dll`" `"`$`(ProjectDir`)\bin\$`(ConfigurationName`)\`" /Y /I","");
}
elseif($osType -eq "32-bit")
{
	$buildEventVal=$buildEventVal.Replace("`r`nxcopy `"$installPath\additionalLib\Oracle20\x32\Oracle.DataAccess.dll`" `"`$`(ProjectDir`)\bin\$`(ConfigurationName`)\`" /Y /I","");
}
else
{
